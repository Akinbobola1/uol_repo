...
Drupal.behaviors.basic = {
  attach: function(context, settings) {
    ...
    //Resize navigation height back to orignal    
    ...
    $('.view-home-image .tab-content').hide(); 
    $('.view-home-image .tab-content:first').show(); 
    $('.view-home-image .view-content > div:first-child .tab-text').addClass("active");
    $('.view-home-image .tab-text').on('click', function(e) { 
    ...
    //Responsive menu
    $("#off-canvas-left-show").on("click",function(e){
        e.preventDefault();
        $(this).toggleClass("push-right");
        $("#navigation").toggleClass("nav-on");
        
        //Fetch window height and apply to responsive navi
        //$(".nav-on").css("height", $(document).height());
    ...
    //Responsive 2nd level menu tree   
    $('li.expanded').on("click",function(){
    $('li.expanded ul.menu').not(this).removeClass('menu-tree-open');    
    $(this).toggleClass('menu-tree-open');  
    });
    
    //Fetch window height and apply to responsive navigation
    //$(".nav-on").css("height", $(document).height());
    ...
    $("#off-canvas-right-show, .change-filters").on("click",function(){
       
        $(".page-properties .sidebar").toggleClass("search-on");
        
        //Fetch window height and apply to responsive navi
        $(".search-on").css("height", $(document).height());
        
    });
        
    //Responsive SEARCH menu
    $("#off-canvas-right-show, .change-filters").on("click",function(){
        $("#off-canvas-right-show").toggleClass("push-left");
    });
    
        
    //Property results : results & Map buttons toggle class
    var mapList = localStorage['mapList'];
    var header = $('.page-properties .view-header').text();
    var headerMap = header.substr(header.indexOf('of') + 3, header.length);

    if (mapList == 1) {
        $('.page-properties #content-area .inside > .item-list li.last').addClass('active');
        // hide sort and paging
        $('.page-properties .view-header').text(headerMap);
        $('.page-properties .views-widget-per-page').hide();
        $('.page-properties .views-widget-sort-sort_bef_combine').hide();

    } else {
        $('.page-properties #content-area .inside > .item-list li.first').addClass('active');
    }
    
        
    $('.page-properties #content-area .inside > .item-list li.first .pane-title').on('click',function(){
        $('.page-properties #content-area .inside > .item-list li.first').addClass('active');
        $('.page-properties #content-area .inside > .item-list li.last').removeClass('active');
        // show sort and paging
        $('.page-properties .view-header').text(header);
        $('.page-properties .views-widget-per-page').show();
        $('.page-properties .views-widget-sort-sort_bef_combine').show();
        localStorage['mapList'] = 0;
    });
    
    $('.page-properties #content-area .inside > .item-list li.last .pane-title').on('click',function(){
        $('.page-properties #content-area .inside > .item-list li.last').addClass('active');
        $('.page-properties #content-area .inside > .item-list li.first').removeClass('active');
        // hide sort and paging
        $('.page-properties .view-header').text(headerMap);
        $('.page-properties .views-widget-per-page').hide();
        $('.page-properties .views-widget-sort-sort_bef_combine').hide();
        localStorage['mapList'] = 1;
    });
    
        
    
    //Property details : Slider & Map buttons toggle class
    $('.node-type-property .tabs .item-list li.first').addClass('active');
        
    $('.node-type-property .tabs .item-list li.first .pane-title').on('click',function(){
        $('.node-type-property .tabs .item-list li.first').addClass('active');
        $('.node-type-property .tabs .item-list li.last').removeClass('active');   
    });
    
    $('.node-type-property .tabs .item-list li.last .pane-title, .node-type-property .view_map').on('click',function(){
        $('.node-type-property .tabs .item-list li.last').addClass('active');
        $('.node-type-property .tabs .item-list li.first').removeClass('active'); 
        event.preventDefault(); 
    }); 


    // Facets
    $('.facetapi-facetapi-checkbox-links .leaf, .facetapi-facetapi-checkbox-links .expanded.first.last').each(function(){
        //this wrapped in jQuery will give us the current .letter-q div
        $(this).children("input").prependTo(this);
    });
    ...
    //Move results page value
    $(".view-display-id-panel_pane_1 .view-header").prependTo(".search-tabs");  

    
    //Email landlord
    $(".email-landlord").on("click",function(){
        $('#block-webform-client-block-1260').addClass('active');
    });
    $("#block-webform-client-block-1260 .block-title").on("click",function(){
        $('#block-webform-client-block-1260').removeClass('active');
    });

    //Email friend
    $(".email-friend").on("click",function(){
        $('#block-webform-client-block-1301').addClass('active');
    });
    $("#block-webform-client-block-1301 .block-title").on("click",function(){
        $('#block-webform-client-block-1301').removeClass('active');
    });

    // Back to search resaults
    if ($('.page-properties').length) {
        localStorage['backToResults'] = document.URL;
    }

    if ($('.back-results').length) {
        if (localStorage.backToResults) {
            $(".back-results a").attr("href", localStorage.backToResults);
        } else {
            $(".back-results").hide();
        }
    }
    var maxHeight = 0;
    $(".big-menu .big-menu-item").each(function(){
    maxHeight = $(this).height() > maxHeight ? $(this).height() : maxHeight;
    }).height(maxHeight+25);

    $(document).on('click', '#content-area .content a[href^="#"]', function (e) {
        e.preventDefault();
